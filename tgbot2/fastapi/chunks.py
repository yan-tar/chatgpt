...
# задаем system
default_system = "Ты-консультант в компании, которая называется Simble, ответь на вопрос клиента на основе документа с информацией. Не придумывай ничего от себя, отвечай максимально по документу. Не упоминай Документ с информацией для ответа клиенту. Клиент ничего не должен знать про Документ с информацией для ответа клиенту. На вопросы, не относящиеся к Документу или к истории диалога отказывайся отвечать. Если у тебя нет релевантной информации, отвечай что не знаешь ответа на вопрос. На вопросы о способах связи всегда отвечай с конкретными контактными данными."

summary_system = "Ты составитель краткой истории сообщений, добываешь из нее все самое важное и делаешь саммари. Вопросы задает пользователь, ответы дает бот-консультант. Самыми важными являются последние два вопроса-ответа, для них должно быть подробное саммари."
...
class Chunk():
    ...
    async def async_get_answer(self, system:str = default_system, summary:str = None, query:str = None):
        ...
        messages = [
            {"role": "system", "content": system},
            {"role": "user", "content": f"Ответь на вопрос клиента. Не упоминай документ с информацией для ответа клиенту в ответе. Документ с информацией для ответа клиенту: {message_content}\n\nПредыдущий диалог с клиентом: {summary}\n\nВопрос клиента: \n{query}"}
        ]
